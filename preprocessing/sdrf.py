...
def sdrf_rewire(pos, edge_index, G, loops=10, remove_edges=False, removal_bound=0.5, tau=1, is_undirected=False):
    # N = data.x.shape[0]
    N = pos.shape[0]
    ...
    edges = list(G.edges())
    edges = np.transpose(np.array(edges)).astype(np.int32)
